...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: rawLocale === 'en-US' ? 'Projects' : '项目',
    description:
      rawLocale === 'en-US'
        ? "Nicolas Leigh's full-stack, Go backend, realtime and mobile engineering projects."
        : '李林泽的全栈、Go 后端、实时系统和移动应用项目实践。',
    alternates: {
      canonical: localizedPath(rawLocale, '/projects'),
      languages: { 'zh-CN': '/zh-CN/projects', 'en-US': '/en-US/projects', 'x-default': '/zh-CN/projects' },
    },
  };
}
      ...
      <main id='main-content' className='projects-page page-shell'>
        <header className='projects-hero page-intro'>
          ...
          <div>
            <p>
              {en
                ? 'System design, core data flows, and practical AI delivery across modern web and concurrent backends.'
                : '记录现代全栈 Web、高并发后端与 AI 工具链的系统设计、核心数据流与落地实践。'}
            </p>
            <dl>
              <div>
                <dt>WEB</dt>
        ...
        <section className='project-cases motion-section' aria-labelledby='project-cases-title'>
              ...
              <li key={project.slug}>
                <article className='case-study'>
                  ...
                  <header className='case-title'>
                    ...
                    <ViewTransition name={`project-title-${project.slug}`} share='title-morph' default='none'>
                      <h3>
                        {project.caseStudyPath ? (
                          <Link href={localizedPath(locale, project.caseStudyPath)}>{project.name}</Link>
                        ) : (
                          <a href={project.website ?? project.repository} rel='noreferrer' target='_blank'>
                            {project.name}
                          </a>
                        )}
                      </h3>
