  ...
  return (
    <PageTransition>
      <main id="main-content">
        <section className="hero page-shell" aria-labelledby="hero-title">
          ...
          <div className="hero-copy">
            <p className="eyebrow reveal reveal-1">
              {siteContent.introduction.eyebrow}
            </p>
            <h1 id="hero-title" className="display-title reveal reveal-2">
              {en ? <><span>Turning complex systems</span><span>into clear and</span><span>reliable products.</span></> : <><span>把复杂系统，</span><span>做成清晰而</span><span>可靠的产品。</span></>}
            </h1>
            <p className="hero-description reveal reveal-3">
              {siteContent.introduction.body}
            </p>

            <div className="hero-actions reveal reveal-4">
              <Link className="primary-link" href={localizedPath(locale, "/posts")}>
                {en ? "Read my writing" : "阅读技术文章"} <span aria-hidden="true">↗</span>
              </Link>
        ...
        <section
          id="about"
          className="statement-section page-shell motion-section"
          aria-labelledby="about-title"
        >
          <p className="section-kicker">01 / APPROACH</p>
          <div>
            <h2 id="about-title">{en ? "I care about how systems evolve, not just what ships today." : "不仅交付页面，也关心系统如何长期演进。"}</h2>
            <p>{en ? "From interface hierarchy and server data contracts to caching, deployment and observability, I treat product experience and engineering quality as the same problem." : "从界面信息层级、服务端数据契约，到缓存、部署与可观测性，我倾向于把产品体验和工程质量视为同一个问题。"}</p>
          </div>
        </section>

        <section
          id="work"
          className="work-section page-shell motion-section"
